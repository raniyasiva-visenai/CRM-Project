"""
VNR Homes CRM Builder (vnrhomes.in)
Platform: ASP.NET WebForms — Playwright UI automation
Similar pattern to DAC Promoters builder.
"""
...
class VNRBuilder(BaseBuilder):
    """
    VNR Homes Channel Partner CRM.
    ASP.NET WebForms with Select2 and multiselect dropdowns.

    Required config:
      - login_url     : https://vnrhomes.in/Cp_Lead/
      - submit_url    : https://vnrhomes.in/Cp_Lead/Marketing_CPlead.aspx
      - username / password
      - project       : e.g. "Tiara"
      - location      : e.g. "Thazhambur"
      - property_type : e.g. "Flats"
    """

    def __init__(self, config: Dict[str, Any], session_manager=None):
        super().__init__(config, session_manager=session_manager)
        self.login_url     = config.get("login_url", "https://vnrhomes.in/Cp_Lead/")
        self.submit_url    = config.get("submit_url", "https://vnrhomes.in/Cp_Lead/Marketing_CPlead.aspx")
        self.username      = config.get("username")
        self.password      = config.get("password")
        self.project       = config.get("project", "Tiara")
        self.location      = config.get("location", "Thazhambur")
        self.property_type = config.get("property_type", "Flats")
    ...
    def submit_lead(self, lead: Lead) -> Dict[str, Any]:
        ...
        with sync_playwright() as p:
            self.log("🚀 Launching browser for VNR Homes submission...")
            ...
            try:
                ...
                if "Marketing_CPlead" not in page.url:
                    self.log("⚠️ No auto-redirect, navigating to lead form...")
                ...
                try:
                    ...
                    return {"success": False, "error": "VNR lead form not found"}
                ...
                page.fill("input[name='ctl00$ContentPlaceHolder1$txtemail']", lead.email or "")
                ...
                if any(w in body.lower() for w in ["success", "submitted", "saved", "thank"]):
                    ...
                    return {"success": True, "status": "CREATED", "message": "Lead submitted to VNR Homes"}

                if "already exist" in body.lower() or "duplicate" in body.lower():
                    self.log("⚠️ Duplicate lead")
                    return {"success": True, "status": "DUPLICATE", "message": "Lead already exists in VNR"}

                if any(w in body.lower() for w in ["error", "invalid", "required"]):
                    self.log("❌ Submission error")
                    return {"success": False, "error": f"VNR validation error: {body[:200]}"}

                # Check if form was reset (cleared = success)
                try:
                ...
                return {"success": False, "error": "Unclear VNR submission result"}

...
class VGKBuilder(BaseBuilder):
    """
    VGK Builders Channel Partner CRM Builder.
    VGK uses a Sell.do portal.
    ...
    def __init__(self, config: Dict[str, Any], session_manager=None):
        ...
        self.login_url = config.get("login_url", "https://cp.vgkbuilders.in/users/sign_in")
        self.submit_url = config.get("submit_url", "https://cp.vgkbuilders.in/dashboard?locale=en&remote-state=/admin/leads/new?locale=en&role=user")
        self.username = config.get("username")
        self.password = config.get("password")
        self.project_label = config.get("project", "VGK Cholavanam")
        self.project_id = config.get("crm_project_id", "687242259f967d52fffea25b")
    ...
    def submit_lead(self, lead: Lead) -> Dict[str, Any]:
        ...
        with sync_playwright() as p:
            self.log("🚀 Launching browser for VGK Builders (Sell.do) submission...")
            ...
            try:
                ...
                if "/users/sign_in" in page.url:
                    ...
                    return {"success": False, "error": "VGK login failed — check credentials"}

                self.log(f"✅ Logged in! Current URL: {page.url}")

                # ── 2. NAVIGATE TO LEAD FORM ─────────────────────
                self.log(f"📋 Navigating to lead form: {self.submit_url}")
                # Sometimes Sell.do uses client-side routing, let's try direct navigation first
                page.goto(self.submit_url, wait_until="networkidle", timeout=60000)
                page.wait_for_timeout(5000) # Give modal time to load
                ...
                if "already exist" in page_text.lower():
                    self.log("⚠️ Lead already exists in VGK CRM.")
                    return {
                        "success": True,
                        "status": "DUPLICATE",
                        "message": "Lead already exists in VGK (Sell.do) CRM"
                    }
                ...
                for r in reversed(captured):
                    try:
                        ...
                        if "already exist" in msg or "duplicate" in msg:
                            self.log("⚠️ Duplicate lead captured from API response.")
                            return {"success": True, "status": "DUPLICATE", "response": data}
                    except:
                        pass

                # Fallback check for modal closure or success message
                ...
                self.log(f"❌ Error during VGK submission: {e}")
                try:
                    page.screenshot(path=f"vgk_error_{lead.id}.png")
                except:
                    pass
